(function ($) {
    ...
    $.fn.disableSelection = function () {
        return this.each(function () {
            $(this).attr('unselectable', 'on').css({'-moz-user-select': 'none', '-webkit-user-select': 'none', 'user-select': 'none', '-ms-user-select': 'none'}).each(function () {
    ...
    var tId;
    $.event.special.tripleclick = {setup: function (data, namespaces) {
    ...
    var dId;
    $.event.special.doubleclick = {setup: function (data, namespaces) {
        var elem = this, $elem = jQuery(elem);
    ...
    var StyleSheet = {StyleCache: {}, getAllStyles: function (selector, parent) {
    ...
    $.each(types, function (i, t) {
        if (t !== SVGRectElement && t !== SVGImageElement) {
            $.extend(t.prototype, {width: function () {
        ...
        $.extend(t.prototype, {offset: function () {
            if (t === SVGTSpanElement) {
                ...
                return{left: this.getCTM().e + num(this.getAttribute('x')), top: this.getCTM().f + num(this.getAttribute('y'))}
            }
            return{left: this.getCTM().e - this.parentNode.getCTM().e + num(this.getAttribute('x')), top: this.getCTM().f - this.parentNode.getCTM().f + num(this.getAttribute('y'))}
        }, position: function () {
            if (t === SVGTSpanElement) {
                ...
                return{left: Math.round(screenCTM.e + (x + dx) * screenCTM.a), top: Math.round(screenCTM.f + (y + dy) * screenCTM.d)}
            }
            var pos;
    ...
    $.extend(SVGInputElements.prototype, {textArea: function (parent, x, y, value, settings) {
        ...
        return this._textArea(args.parent, args.value, $.extend({x: (args.x && isArray(args.x) ? args.x.join(' ') : args.x), y: (args.y && isArray(args.y) ? args.y.join(' ') : args.y)}, args.settings || {}));
        ...
        return this._list(args.parent, args.value, $.extend({x: (args.x && isArray(args.x) ? args.x.join(' ') : args.x), y: (args.y && isArray(args.y) ? args.y.join(' ') : args.y)}, args.settings || {}));
        ...
        return this._text(args.parent, args.value, $.extend({x: (args.x && isArray(args.x) ? args.x.join(' ') : args.x), y: (args.y && isArray(args.y) ? args.y.join(' ') : args.y)}, args.settings || {}));
        ...
        return this._image(args.parent, args.value, $.extend({x: (args.x && isArray(args.x) ? args.x.join(' ') : args.x), y: (args.y && isArray(args.y) ? args.y.join(' ') : args.y)}, args.settings || {}));
    ...
    $.extend(SVGSelectableGElement, {_once: false, _instances: [], setup: function (instance) {
    ...
    $.extend(SVGSelectableGElement.prototype, {_group: null, selected: false, _class: 'selectable', _events: null, _parent: null, _destroyed: false, bind: function () {
        ...
        if (this._wrapper && !this._destroyed) {
            ...
            $.extend(this._group, {_wrapper: this._wrapper, _selectable: this, _selected: this.selected, select: function (e) {
                if (!this._selected)
                    SVGSelectableGElement.deselectAll();
                this._selected = true;
    ...
    $.extend(SVGEditableTextBox, {once: false, _wordCache: {}, _textareaCount: 0, setup: function () {
        if (!this.once) {
                ...
                if (selectedGroup && selectedGroup.constructor === SVGEditableTextBox) {
                    ...
                    if (e.metaKey || e.ctrlKey) {
                        ...
                        switch (e.keyCode) {
                            ...
                            case 65:
                                ...
                                selectedGroup._selection = {start: selectedGroup._selectStartCoord, stop: stopcoord};
                    ...
                    if (cancelUpdate && !markall) {
                        ...
                        if (selectedGroup._group._selected && !unselect_marker) {
                            ...
                            SVGTextMarker.show(selectedGroup._wrapper, $.extend(coord, {width: 2 / selectedGroup._group.getCTM().a, height: lineHeight * 1.2, desx: desx}));
                        }
                        if (selectedGroup._selectStartCoord) {
    ...
    $.extend(SVGEditableTextBox.prototype, {_text: '', _textPosition: 0, _selectStartCoord: null, _selection: null, _selectionDisabled: false, _tplClickState: false, _keepDesiredX: false, _moveDown: true, _renderTimer: -1, _contextMenu: false, _classType: "textbox", _size: {width: 0, height: 0}, g: null, _history: [
        {}
    ], _historyPos: 0, _historyAdd: function (val, textPosition) {
        ...
        this._history = [
            {text: value.toString(), textPosition: null}
        ];
        this._width = width;
        this._height = height;
        ...
        var padding = {'top': num(StyleSheet.get('rect.background', 'padding-top', g)) * 1.2, 'right': num(StyleSheet.get('rect.background', 'padding-right', g)), 'bottom': num(StyleSheet.get('rect.background', 'padding-bottom', g)), 'left': num(StyleSheet.get('rect.background', 'padding-left', g))}
        return padding;
        ...
        if (g) {
                            ...
                            if (tmpRow == '') {
                                ...
                                while (remainingChars[0]) {
                                    ...
                                    if (cachedWord === undefined) {
                                        ...
                                        cachedWord = SVGEditableTextBox._wordCache[fontSettings][newTmpWord] = {width: tmpText.width()
                                            + padding['left']
                                            + padding['right'], };
                                    }
            ...
            if (g._selected && !this._selectionDisabled) {
                ...
                SVGTextMarker.show(this._wrapper, $.extend(coord, {width: 2 / g.getCTM().a, height: lineHeight * 1.2, desx: desx}));
            }
        ...
        if (row && row.firstChild) {
            ...
            return{parent: g, element: row, x: Math.round((closestPos - screenCTM.e) / screenCTM.a), y: Math.round((pos.top - screenCTM.f) / screenCTM.d) - lineHeight, paragraph: paragraphIndex + 1, row: rowIndex + 1, char: i - 1};
        }
        return null;
    }, _getClosestRowCoordsInText: function (g, e) {
        var pos = this._coordInText(g, e);
        if (pos) {
            return{parent: g, element: pos.element, x: pos.x, y: pos.y, paragraph: pos.paragraph, row: pos.row, char: pos.char};
        }
        return null;
        ...
        if (pos) {
            ...
            return{start: {parent: g, x: pos.x - len1, y: pos.y, paragraph: pos.paragraph, row: pos.row, char: i}, stop: {parent: g, x: pos.x + len2, y: pos.y, paragraph: pos.paragraph, row: pos.row, char: j}};
        }
        return null;
        ...
        if (g !== undefined && g !== null) {
            ...
            if (pEl) {
                ...
                if (rEl && rEl.firstChild) {
                    ...
                    return{parent: g, element: row, x: Math.round((charPos - screenCTM.e) / screenCTM.a), y: Math.round((pos.top - screenCTM.f) / screenCTM.d) - lineHeight, paragraph: paragraph, row: row, char: char};
                }
            }
        }
        return null;
        ...
        if (g) {
            ...
            if (pEl) {
                ...
                if (rEl && rEl.firstChild) {
                    ...
                    return{parent: g, element: rEl, x: Math.round((closestPos - screenCTM.e) / screenCTM.a), y: Math.round((pos.top - screenCTM.f) / screenCTM.d) - lineHeight, paragraph: paragraph - 1, row: row - 1, char: i - 1};
                }
            }
        }
        return null;
        ...
        marking.parentNode.insertBefore(marking, marking.parentNode.firstChild.nextSibling);
        this._selection = {start: {parent: g, element: coords.element, x: pos.left, y: pos.top - height, paragraph: coords.paragraph, row: coords.row, char: 0}, stop: {parent: g, element: coords.element, x: pos.left + width, y: pos.top - height, paragraph: coords.paragraph, row: coords.row, char: coords.element.firstChild.data.length - 1}};
                    ...
                    $.each(selectStopCoord.parent.getElementsByTagName('text'), function (i, el) {
                        ...
                        $.each(el.childNodes, function (i2, el2) {
                            ...
                            if (that._selectStartCoord.paragraph > selectStopCoord.paragraph) {
                                ...
                                minRow = {p: that._selectStartCoord.paragraph, r: Math.min(that._selectStartCoord.row, selectStopCoord.row)};
                                maxRow = {p: that._selectStartCoord.paragraph, r: Math.max(that._selectStartCoord.row, selectStopCoord.row)};
            ...
            if (e.button != 2) {
                ...
                if (!diff || (diff > 300 || this._selection) && !this._tplClickState) {
                    ...
                    if (!this._selectionDisabled) {
                        SVGTextMarker.show(this._wrapper, $.extend(coord, {width: 2 / g.getCTM().a, height: lineHeight * 1.2, desx: coord.x}));
                    }
                    row = coord.row - 1;
        ...
        if (this._selectStartCoord) {
            ...
            if ((dy > lineHeight || dx > 3)) {
                ...
                SVGTextMarker.show(this._wrapper, $.extend(this._selectStartCoord, {width: 2 / g.getCTM().a, height: lineHeight * 1.2, desx: this._selectStartCoord.x}));
                $('.marking').remove();
                this._selection = null;
            }
        }
    ...
    $.extend(SVGEditableList.prototype, {_classType: "list", _getGPadding: function (g) {
        var padding = {};
    ...
    $.extend(SVGEditableText.prototype, {_classType: "text", _preProcessSetText: function (text, textPosition) {
    ...
    SVGTextMarker = {};
    $.extend(SVGTextMarker, {_wrapper: null, _marker: null, _visible: false, _blinkId: -1, _blinkState: true, _settings: {parent: null, x: 10, y: 10, width: 2, height: 12, paragraph: 0, row: 0, char: 0, desx: 0}, show: function (wrapper, options) {
        this._visible = true;
        this._wrapper = wrapper;
    ...
    $.extend(SVGEditableImage, {once: false, setup: function () {
        if (!this.once) {
            $(window).bind('keydown.' + this.name, function (e) {
                var selectedGroup = SVGSelectableGElement.selectedGroup();
                var self = this;
    ...
    $.extend(SVGEditableImage.prototype, {_classType: "image", _renderTimer: -1, _height: 0, _width: 0, init: function (parent, value, width, height, settings) {
        var self = this;
        this._parent = parent;
