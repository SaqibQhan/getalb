class AlbumesController < ApplicationController
  ...
  def create_album
    ...
    for i in 1..shapes_count
      ...
      album_page = AlbumDetail.new(:album_id => album.id, :template_id => params[:template_id], :page => params[:page], :rotate => params["image#{i}"], :scale => params["scale#{i}"].to_f, :shape_id => params["shape_id#{i}"], :top => params["top#{i}"].to_f, :left => params["left#{i}"].to_f)
  ...
  def update_album
    ...
    for i in 1..shapes_count
      image_type = params["image_type#{i}"]
      album_detail = AlbumDetail.find_by_album_id_and_shape_id_and_page(album.id, params["shape_id#{i}"], params[:page])
      album_page = album_detail.update_attributes(:template_id => params[:template_id], :rotate => params["image#{i}"], :scale => params["scale#{i}"].to_f, :shape_id => params["shape_id#{i}"], :top => params["top#{i}"].to_f, :left => params["left#{i}"].to_f)
